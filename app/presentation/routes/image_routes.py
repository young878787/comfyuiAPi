...
from app.application.dtos.common import ImageResponse, ImageListResponse, OpenFolderRequest
...
async def view_image(date_str: str, filename: str, service: ImageService = Depends(get_image_service)):
    """View an image."""
    try:
        ...
        return FileResponse(
            image_path, media_type="image/png", headers={"Cache-Control": "public, max-age=31536000, immutable"}
        )
...
async def download_image(date_str: str, filename: str, service: ImageService = Depends(get_image_service)):
    """Download an image."""
    try:
        image_path = service.get_image_path(date_str, filename)
        if not image_path.exists():
...
async def list_images(date_str: str, service: ImageService = Depends(get_image_service)):
    ...
    try:
        ...
        for filename in images:
            ...
            images_details.append(
                ImageResponse(filename=filename, url=f"/api/image/view/{date_str}/{filename}", metadata=meta_dict)
            )

        return ImageListResponse(date=date_str, images=images, latest_image=latest, images_details=images_details)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/metadata/{date_str}/{filename}", response_model=ImageResponse)
async def get_image_metadata(date_str: str, filename: str, service: ImageService = Depends(get_image_service)):
    ...
    try:
        ...
        return ImageResponse(
            filename=filename, url=f"/api/image/view/{date_str}/{filename}", metadata=metadata.to_dict()
        )
...
async def open_folder(request: OpenFolderRequest, service: ImageService = Depends(get_image_service)):
...
async def delete_image(date_str: str, filename: str, service: ImageService = Depends(get_image_service)):
